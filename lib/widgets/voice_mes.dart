...
class _MyVoiceMessageState extends State<MyVoiceMessage>
  ...
  void initState() {
    ...
    // _player.setAudioSource(AudioSource.uri(Uri.parse(widget.audioSrc),tag: item,), initialIndex: 0, initialPosition: Duration.zero);
    _player.global.setGlobalAudioContext(ctx);
    print("AudioContextSet");
  ...
  String FormatSmallDur(int chislo){
    return chislo<9 ? "0"+chislo.toString():chislo.toString();
  }

  void _setAnimationConfiguration(Duration audioDuration) async {

    // widget.formatDuration != null?(audioDuration);

    setState(() {
      _remainingTime = FormatSmallDur(audioDuration.inMinutes)+":"+FormatSmallDur(audioDuration.inSeconds);
    });
